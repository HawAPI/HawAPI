...
class SoundtrackControllerTest {
    ...
    void shouldReturnAllSoundtracks() throws Exception {
        ...
        List<SoundtrackDTO> res = Collections.singletonList(soundtrack);
        Page<UUID> uuids = PageableExecutionUtils.getPage(
                Collections.singletonList(soundtrack.getUuid()),
                pageable,
                () -> 1
        );
        HttpHeaders headers = buildHeaders(uuids, null);
        ...
        when(service.findAll(any())).thenReturn(res);
    ...
    void whenNoUUIDIsFoundShouldReturnEmptyList() throws Exception {
        Pageable pageable = Pageable.ofSize(1);
        List<SoundtrackDTO> res = Collections.emptyList();
        Page<UUID> uuids = PageableExecutionUtils.getPage(
